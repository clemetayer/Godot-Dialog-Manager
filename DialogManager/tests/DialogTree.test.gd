...
##### VARIABLES #####
var dialog_box_template_path = "res://tests/DialogBox.tscn"
var dialog_box_choices_path = "res://tests/DialogBoxChoices.tscn"
var dialog_box
...
func checkDialogStarted(dialog_node, dialog_box):
	asserts.is_true(dialog_node.isStarted,"Dialog node isStarted true for " + dialog_node.DIALOG_KEY)
	asserts.is_true(dialog_box.dialog_started, "dialog_started true for " + dialog_node.DIALOG_KEY)
	asserts.is_true(dialog_box.visible,"Dialog box visible for " + dialog_node.DIALOG_KEY)
	asserts.is_equal(dialog_box.DIALOG, tr(dialog_node.DIALOG_KEY), "Dialog correctly set for " + dialog_node.DIALOG_KEY)

# shortcut for testing that a dialog is done
func checkDialogDone(dialog_node,dialog_box):
	asserts.is_false(dialog_node.get_node(dialog_node.BOX_PATH).is_connected("dialog_done",dialog_node,"dialogDone"), "dialog_done disconnected correctly for " + dialog_node.DIALOG_KEY)
	asserts.is_false(dialog_node.isStarted,"Dialog node isStarted false for " + dialog_node.DIALOG_KEY)
	asserts.signal_was_emitted(dialog_node,"dialog_done","dialog_done signal was emitted for " + dialog_node.DIALOG_KEY)
