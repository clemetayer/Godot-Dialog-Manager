...
func stopDialog():
	...
	RTL.set_visible_characters(0)
	RTL.set_bbcode(dialog_pages[page])
	test_emulate_next_dialog_key = false
...
func processFunction(_delta):
	if(dialog_started):
		if((Input.is_action_just_pressed("next_dialog") and RTL.get_visible_characters() > 0) or test_emulate_next_dialog_key): # RTL.get_visible_characters() > 0 to avoid accidentally skipping the dialog on dialog switch
