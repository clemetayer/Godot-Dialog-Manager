...
func startDialog():
	...
	if(node != null):
		if(GlobalUtils.checkIfHasSignal(node,"dialog_done") and not node.is_connected("dialog_done", self, "dialogDone")):
...
func disconnectSignals():
	var node = get_node_or_null(BOX_PATH)
	if(node != null):
		if(GlobalUtils.checkIfHasSignal(node, "dialog_done") and node.is_connected("dialog_done",self,"dialogDone")):
			node.disconnect("dialog_done",self,"dialogDone")
		if(GlobalUtils.checkIfHasSignal(node, "choice_made") and node.is_connected("choice_made",self,"choiceMade")):
